...
internal override IEnumerable<Row> GetEntities(DataExtractorHelper schema)
		{
			foreach (var record in page.Records)
			{
				...
				try
				{
					dataRow = ParseRecord(record, schema, compression);
				}
				catch (Exception ex)
				{
					System.Diagnostics.Debug.WriteLine("AVISO: registro corrompido ignorado (corrupcao): " + ex.Message);
					continue;
				}
		...
		private static Row ParseRecord(PrimaryRecord record, DataExtractorHelper schema, CompressionContext compression)
		{
				short fixedOffset = 0;
