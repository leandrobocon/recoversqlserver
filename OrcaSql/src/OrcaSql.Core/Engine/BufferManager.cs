...
namespace OrcaSql.Core.Engine
{
    ...
    public class BufferManager// : IDisposable
    {
        ...
        internal byte[] GetPageBytes(short fileID, long pageID, bool putResultToCache)
        {
            ...
            {
                Console.Error.WriteLine("AVISO: pagina referenciada em arquivo inexistente (fileID=" + fileID + ", pageID=" + pageID + ") - retornando pagina vazia (corrupcao).");
                var empty = new byte[8192];
                if (putResultToCache)
                    buffer.Add(fileID, new Dictionary<long, byte[]>());
                return empty;
            }
